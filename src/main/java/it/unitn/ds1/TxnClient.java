...
public class TxnClient extends AbstractActor {
	...
	private static final int MAX_TXN_LENGTH = 12;
	...
	private static final Logger log = LogManager.getLogger(TxnClient.class);
	...
	// send this message to the client at startup to inform it about the
	// coordinators and the keys
	...
	// start a new TXN: choose a random coordinator, send TxnBeginMsg and set
	// timeout
	void beginTxn() {
		...
		acceptTimeout = getContext().system().scheduler().scheduleOnce(Duration.create(500, TimeUnit.MILLISECONDS),
				getSelf(), new TxnAcceptTimeoutMsg(), // message sent to myself
	...
	void endTxn() {
		...
		firstValue = null;
		secondValue = null;
		log.info("client" + clientId + " END");
	}

	// READ two items (will move some amount from the value of the first to the
	// second)
	void readTwo() {
		...
		log.info("client" + clientId + " READ #" + numOpDone + " (" + firstKey + "), (" + secondKey + ")");
	}

	// WRITE two items (called with probability WRITE_PROBABILITY after readTwo()
	// values are returned)
	void writeTwo() {
		...
		if (firstValue >= 1)
			amountTaken = 1 + r.nextInt(firstValue);
		...
		log.info("client" + clientId + " WRITE #" + numOpDone + " taken " + amountTaken + " (" + firstKey + ", "
				+ (firstValue - amountTaken) + "), (" + secondKey + ", " + (secondValue + amountTaken) + ")");
	}
	...
	private void onReadResultMsg(ReadResultMsg msg) {
		log.info("client" + clientId + "<--[READ(" + msg.key + ")=" + msg.value + "]");

		// save the read value(s)
		if (msg.key.equals(firstKey))
			firstValue = msg.value;
		if (msg.key.equals(secondKey))
			secondValue = msg.value;
	...
	private void onTxnResultMsg(TxnResultMsg msg) throws InterruptedException {
		if (msg.commit) {
			numCommittedTxn++;
			log.info("client" + clientId + "<--[COMMIT OK(" + numCommittedTxn + "/" + numAttemptedTxn + ")]");
		} else {
